# 🛒 Shopping List — Weeks 13–14: Module 2 Capstone: Poultry & Flavor Review

← [Back to Full Guide](index.md) | [Back to Quick Ref](quick-ref.md) | [Back to Master Index](../../../README.md)

---

> **How to use this list:** Week 13 shopping depends on **your** menu choice for Saturday's dinner. The list below covers the suggested menu (shrimp cocktail + duck + polenta) plus Week 13's fixed weeknight meals. Week 14 content is coming soon.

---

## 🥩 Proteins — Week 13 Weeknights

- [ ] Ground pork or ground chicken — 1 lb (Week 13 Wed Larb)
- [ ] **Duck breasts — 2–4 (Week 13 Sat suggested main)** — found at specialty butchers and some grocery stores (Moulard or Pekin duck breasts both work)
- [ ] Large shrimp (shell-on, 16/20 count) — 1 lb (Week 13 Sat suggested starter)

---

## 🥩 Proteins — Week 13 Saturday Dinner (choose your menu)

*If using the suggested framework:*
- [ ] 2–4 duck breasts, skin-on (main course)
- [ ] 1 lb large shrimp, shell-on (cocktail starter — cook shell-on, then peel)

*If making your own menu (based on Weeks 1–12 techniques):* — write your list here —

---

## 🥦 Produce — Week 13

**Wednesday Larb:**
- [ ] Fresh mint — 1 large bunch
- [ ] Fresh cilantro — 1 bunch
- [ ] Shallots — 4 (thin-sliced)
- [ ] Thai bird chilies or red Fresno chilies — 3–4
- [ ] Limes — 4 (fresh lime juice)
- [ ] Jasmine rice — 1 cup (also for toasted rice powder)
- [ ] Cucumbers — 2 (quick pickle)
- [ ] Rice wine vinegar — 1 cup (quick pickle)
- [ ] Red onion — 1 (quick pickle)

**Friday:**
- [ ] Good bread — 1 loaf (grilled cheese)
- [ ] Tomatoes (canned) — 2 cans (tomato soup)
- [ ] Yellow onion — 1 (soup)

**Saturday dinner:**
- [ ] Root vegetables (carrots, parsnips, turnips) — 2 lbs (suggested main side)
- [ ] Cherries (fresh or preserved) or quality cherry preserves — ½ cup (duck pan sauce)
- [ ] Fresh thyme — 1 bunch
- [ ] Shallots — 4 additional (duck pan sauce)

---

## 🧀 Dairy

- [ ] Good cheese for a cheese board (1–2 varieties) — Week 13 Sat optional third course
- [ ] Unsalted butter — 2 sticks (duck pan sauce, polenta finishing)
- [ ] Parmesan — 4 oz (polenta)
- [ ] Heavy cream — ½ cup (grilled cheese, optional; polenta finishing)
- [ ] Nice cheddar or Gruyère — 6 oz (grilled cheese)

---

## 🌾 Pantry & Dry Goods

- [ ] Coarse-ground cornmeal (not instant) — ½ bag (Week 13 Sat polenta)
- [ ] Chicken stock — 32 oz (polenta + duck pan sauce)
- [ ] Good red wine — 1 bottle (duck pan sauce + table)
- [ ] Fish sauce — 1 bottle (Week 13 Wed Larb — already in pantry from Week 10?)
- [ ] Shrimp cocktail sauce: horseradish, ketchup, Worcestershire, Tabasco, lemon
- [ ] Jasmine rice — 1 cup + extra for toasting (toasted rice powder for larb)
- [ ] Honey — 1 jar (Week 13 Sat cheese board)
- [ ] Good crackers or baguette (cheese board)
- [ ] Honeycomb (Week 13 Sat cheese board — optional but impressive)

---

## 📝 Notes

**Larb (Week 13 Wed) — toasted rice powder:** Toast 3 tbsp raw jasmine rice in a dry skillet over medium heat until golden, stirring constantly. Cool completely, then grind in a spice grinder or mortar and pestle to a coarse powder. This is a classic Thai technique and makes a significant difference in texture and flavor.

**Duck breast — the sear:** Score the fat cap in a crosshatch pattern without cutting into the meat. Start in a cold pan (no oil) over low heat. Render the fat slowly for 8–10 minutes until most of it has melted and the skin is golden. Then flip once and finish in the oven at 400°F for 5–7 minutes. Rest 5 minutes. Duck is best medium-rare (130–135°F internal).

**Week 13 Saturday rule:** Everything is prepped and mis en placed before service starts. Once the first component goes on heat, no recipe tabs. You know what you're doing. Trust it.

**Week 14 (Poultry in Depth)** — shopping list coming when content is finalized. You'll need: whole chickens, duck legs for confit, duck fat (or buy), and a selection of poultry for roasting experiments.
